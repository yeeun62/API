...
module.exports = {
    ...
    postCanvas: async (req, res) => {
        let uuid = await axios.get(`${process.env.SERVER_URL}/v1/uuid`, {
    ...
    findCanvas: (req, res) => {
        ...
        docClient.query(params, (err, data) => {
            if (err) {
                ...
                res.status(200).json({ message: "성공입니다!", data: data.Items[0].canvas_info });
            }
        });
    },
    ...
    getPixabay: async (req, res) => {
        ...
        try {
            const result = await axios.get(`https://pixabay.com/api/?key=${key}&lang=ko&category=${category[0]}`);
    ...
    searchPixabay: async (req, res) => {
        ...
        try {
            const search = await axios.get(`https://pixabay.com/api/?key=${key}&q=${q}`);
